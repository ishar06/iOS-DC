...
var returnValue = checkAge(age: userInputAge)
print(returnValue!)
...
func cost(name: String) -> Double?{
    if let stock = stock[name], let price = prices[name], stock > 0{
        return price
    }else{
        return nil
    }
...
let functionReturnValue = cost(name: "Juice")
