...
print("We get a compilation error because the variable 'foo' is only defined within the scope of the for-loop.")
...
print("Here we declared and initialized 'x' in the global scope so it is accessible everywhere. In the for-loop, we are modifying the value of 'x' within the scope of the for-loop. In the last print statement, we are using the value of 'x' that was set in the for-loop.")
