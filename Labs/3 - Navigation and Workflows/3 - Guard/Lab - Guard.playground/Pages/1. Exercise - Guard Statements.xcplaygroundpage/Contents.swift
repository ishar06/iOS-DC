...
if let area1 = calculateArea(x: 1.5, y: 2.0) {
    print(area1)
}
if let area2 = calculateArea(x: 1.5, y: -1.0) {
    print(area2)
}
...
if let sum1 = add(a: 3, b: 5) {
    print(sum1)
}
if let sum2 = add(a: 3, b: nil) {
    print(sum2)
}
/*:
...
struct User {
    ...
    func createUser() -> User? {
        guard let firstName = self.firstName, let lastName = self.lastName, let age = self.age else {
            return nil
        }
...
var user: User? = User(firstName: firstNameTextField.text, lastName: lastNameTextField.text, age: ageTextField.text)
